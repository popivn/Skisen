<template>
  <div class="container mx-auto max-w-4xl py-12 px-4 md:px-6 lg:px-8">
    <h1 class="text-3xl font-bold mb-8 text-center">{{ $t('chatbot.title') }}</h1>
      ...
      <div class="h-[480px] flex flex-col">
              ...
              <div class="flex max-w-[80%]" :class="message.role === 'user' ? 'flex-row-reverse' : 'flex-row'">
                ...
                <div class="rounded-lg p-3" :class="message.role === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-900'">
                  ...
                  <div v-if="message.imageUrl">
                    <img :src="message.imageUrl" alt="Generated Image" class="rounded-md max-h-60 w-auto mt-2" />
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="p-4 border-t border-gray-200 ">
          <div class="flex gap-2">
            ...
            <button @click="activateImageMode" :disabled="isLoading" class="bg-green-600 hover:bg-green-700 text-white rounded-lg p-2 flex-shrink-0 disabled:opacity-50 disabled:cursor-not-allowed" :title="$t('chatbot.generateImageButton')">
              <span class="sr-only">{{ $t('chatbot.generateImageButton') }}</span>
              <ImageIcon class="h-5 w-5" />
            </button>
            ...
            <button @click="handleSend" :disabled="isLoading || (input.trim() === '' && attachedImages.length === 0)" class="bg-blue-600 hover:bg-blue-700 text-white rounded-lg p-2 flex-shrink-0 disabled:opacity-50 disabled:cursor-not-allowed" :title="$t('chatbot.sendButton')">
...
const isImageMode = ref(false); // Track if user wants to generate an image
...
const handleSend = async () => {
  ...
  if (isImageMode.value) {
    await generateImage(messageToSend, language);  // If in image mode, call generateImage
  } else {
    await chat(messageToSend, language);  // Otherwise, call chat
  }

  isLoading.value = false;
};

const generateImage = async (messageToSend, language) => {
  try {
    // Send request to generate image
    const response = await axios.post('http://localhost:3000/generate-image', { prompt: messageToSend, language });
    const botMessage = { id: Date.now().toString(), role: 'assistant', content: 'Here is the image you requested:', imageUrl: response.data.imageUrl };
    messages.value.push(botMessage);
    isImageMode.value = false;  // Reset image mode after generating image
  } catch (error) {
    console.error('Image generation error:', error);
    const botMessage = { id: Date.now().toString(), role: 'assistant', content: 'There was an error generating the image. Please try again later.' };
    messages.value.push(botMessage);
    isImageMode.value = false;  // Reset image mode after error
  }
};

const chat = async (messageToSend, language) => {
  try {
